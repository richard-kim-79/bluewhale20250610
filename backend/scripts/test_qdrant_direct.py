#!/usr/bin/env python3
"""
Script to directly test Qdrant Cloud connection and operations.
...
def test_qdrant_connection():
    ...
    try:
        ...
        logger.info(f"Successfully connected to Qdrant Cloud. Available collections: {collection_names}")
        return True, client, settings.QDRANT_COLLECTION_NAME
    except Exception as e:
        logger.error(f"Failed to connect to Qdrant: {e}")
        return False, None, None

def test_collection_operations(client, collection_name):
...
def test_vector_operations(client, collection_name):
...
def main():
    ...
    logger.info("Testing Qdrant Cloud setup for BlueWhale project...")
    
    # Test connection
    connection_success, client, collection_name = test_qdrant_connection()
    if not connection_success:
        logger.error("Qdrant connection test failed")
        return False
    
    # Test collection operations
    if not test_collection_operations(client, collection_name):
        logger.error("Qdrant collection operations test failed")
        return False
    
    # Test vector operations
    if not test_vector_operations(client, collection_name):
